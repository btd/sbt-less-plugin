...
public class LessSource {
    ...
    /**
    *   Return all dependencies of this file
    */
    public List<File> getAllFiles() {
        LinkedList<File> imports = new LinkedList<File>();
        LinkedList<LessSource> notProcessed = new LinkedList<LessSource>();
        notProcessed.add(this);

        while(!notProcessed.isEmpty()) {
            LessSource currentSource = notProcessed.poll();
            imports.add(currentSource.file);
            for (Map.Entry<String, LessSource> entry : currentSource.imports.entrySet()) {
                notProcessed.add(entry.getValue());
            }
        }
        return imports;
    }
